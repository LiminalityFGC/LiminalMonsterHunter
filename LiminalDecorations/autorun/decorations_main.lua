...
local chat = require('LiminalCore.utils.chat')
...
local function get_gambling_quantity()
  local decorations_table = {}
  local quest_rank = current_quest.get_rank()

  ---@param category DecorationCategory
  ---@return quantity integer
  local function gamble(category)
    local odds = model.RankToDecorationCategoryOdds[quest_rank][category]
    return roll_for_many(category, odds)
  end

  for _, category in pairs(model.DecorationCategory) do
    decorations_table[category] = {}
    decorations_table[category].quantity = gamble(category)
  end

  return decorations_table
end

---@return nil
local function update_player_progress()
  config.hunter_rank, config.master_rank = player.get_player_rank(progress_manager)
  configuration.write(config_path, config)
end

---@param category DecorationCategory
---@param quantity integer
---@param quest_rank Rank
---@return nil
local function roll_for_decorations(category, quantity, quest_rank)
  ...
  decoration_values.ids = {}
  decoration_values.name = {}
  decoration_values.mh_index = {}

  local decoration_table_by_rank = model.DecorationTableByRank[category]
  local decoration_reference_table = decoration_table_by_rank[quest_rank]

  for i = 1, quantity do
    local decoration_id = math.random(1, #decoration_reference_table)
    table.insert(decoration_values.ids, decoration_id)
    table.insert(decoration_values.mh_index, decoration_reference_table[decoration_id].mh_index)
    table.insert(decoration_values.name, decoration_reference_table[decoration_id].name)
  end

  return decoration_values
end

---@param decoration_table table
---@return nil
function add_decorations_to_inventory(decoration_table)
  local item_box = data_manager:call('getDecorationsBox()')

  for _, category in pairs(model.DecorationCategory) do
    local decoration_indexes = decoration_table[category].decorations.mh_index

    print('what is going on here?')

    for i = 1, #decoration_indexes do
      print('Count: ' .. i .. ' = ' .. decoration_indexes[i])
...
function generate_chat_message(category, decoration_values)
  local category_to_color = {
    [decoration_category.NORMAL] = 'BLUE',
    [decoration_category.RAMPAGE] = 'RED',
  }

  local quantity_headers = {
    [decoration_category.NORMAL] = 'Normal Decorations found: ',
    [decoration_category.NORMAL] = 'Rampage Decorations found: ',
  }

  -- start here
  function color_message(message)
    local output = '<COL ' .. category_to_color[category] .. '>' .. message .. '<COL>'
    return output
  end
  ...
  table.insert(category_message[category], color_message(quantity_headers[category]))
  table.insert(category_message[category], color_message(tostring(decoration_values[category].quantity)) .. '\n')

  for i = 1, #decoration_values[category].decorations.ids do
    table.insert(pair_buffer, decoration_values[category].decorations.name[i])
  ...
  current_chat[category] = category_message[category]
end

---@return nil
local function create_chat()
  for _, category in pairs(model.DecorationCategory) do
    if #current_chat[category] ~= 0 then
      local message = table.concat(current_chat[category])
      current_chat.print(chat.DisplayWindow.RIGHT_NOTIFICATION, message)
    end
  end
end

---@param  decorations_table table
---@return decorations_table table
local function get_decoration_generation_params(decorations_table)
  local quest_rank = current_quest.get_rank()

  for _, category in pairs(model.DecorationCategory) do
    local quantity = decorations_table[category].quantity
    decorations_table[category].decorations = roll_for_decorations(category, quantity, quest_rank)

    if quest_rank == quest.Rank.MASTER then
      roll_for_decorations(model.DecorationCategory.NORMAL, 2, quest.Rank.HIGH)
      roll_for_decorations(model.DecorationCategory.NORMAL, 2, quest.Rank.LOW)
    end

    if quest_rank == quest.Rank.HIGH then
      roll_for_decorations(model.DecorationCategory.NORMAL, 3, quest.Rank.LOW)
    end
  end
...
local function generate_and_insert_decorations()
  local decorations_table = get_gambling_quantity()
  decorations_table = get_decoration_generation_params(decorations_table)
...
local function check_rewards_on_quest_complete(retval)
  ...
  current_chat = chat.get_instance(chat_manager)
  ...
  decoration_values = generate_and_insert_decorations()
  generate_chat_messages(decoration_values)
  create_chat()

  quest.invalidate_cache()
  chat.invalidate_cache()
