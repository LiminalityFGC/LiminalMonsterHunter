...
-- when implementing the gui, I am going to need a wrapper around config, such that I can pull from either the custom gui config values or the default config values
--
-- Maybe global state flag, and then just ternary that shit
...
-- Completely rethink this idea from the ground up
...
local function check_rewards_on_quest_complete(retval)
  ...
  -- not needed, due to current_quest
	-- init_quest_params()
	update_player_progress()


  -- abstract into a table of blocker conditions
  -- so that I can call the same function and it reads from a table using an enum key to see if we still qualify
  ...
  -- handle this is in the quest module, ie boolean quest.is_player_in_quest()
  if not quest_params then
		return
	end

  -- abstract this into a blocker
	if not quest.is_qualifying() then
		return
	end

-- The problem is I am not grouping like steps.  I should be handing a table between the different domains of business logic, then handing the table of configuration values to a function that operates on them.  Instead each function does too many things, in too many domains.  It is hard to reason about.

  -- Completely rethink this idea from the ground up
	filter_valid_melding_names()

  -- Steal from decorations
	dealer()
  ...
  -- This shouldn't even be necessary, I believe this was the result of incorrectly managing state and allowing a condition where the hook somehow ran twice.  
	toggle_rewards()
end
